...
describe("Chainable API", function () {
  ...
  it("should handle changing domains", function () {
    expect(i18n.translate("test other_domain singular").onDomain("other_domain").fetch()).toEqual(
      "other domain test 1",
    );
  });

  it("should allow you to add plural information in the chain.", function () {
    expect(i18n.translate("test plural %1$d").ifPlural(5, "dont matta").fetch()).toEqual("test_1_plural %1$d");
  });

  it("should take in a sprintf set of args (as array) on the plural lookup", function () {
    expect(i18n.translate("test plural %1$d").ifPlural(5, "dont matta").fetch([5])).toEqual("test_1_plural 5");
    expect(i18n.translate("test plural %1$d %2$d").ifPlural(5, "dont matta %1$d %2$d").fetch([5, 6])).toEqual(
      "dont matta 5 6",
    );
    expect(i18n.translate("test plural %1$d %2$d").ifPlural(1, "dont matta %1$d %2$d").fetch([1, 6])).toEqual(
      "test plural 1 6",
    );
  });
  ...
  it("should handle context information.", function () {
    expect(i18n.translate("test context").withContext("context").fetch()).toEqual("test_1context");
  });
